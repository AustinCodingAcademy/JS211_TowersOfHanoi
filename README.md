...
1. Move blocks/numbers from one column to the next, and have them stacked in order from largest to smallest (example of win, in column b || c must be stacked 4,3,2,1)

Code needed:

1. must have function to move block
   1. select by using stacks.object(object coming from towersofhanoi function?)[index]
   2. remove last number from array of the object
2. must have function to check if move is legal
   1. must be an if function
   2. must check if user inputs in startStack and endStack are valid inputs
3. must have function to check for a win
   1. check for 2 wins
      - if user gets numbers in row b in order 4,3,2,1
      - Master Win = numbers in row c in order 4,3,2,1
4. must have a final function to bring all functions together to play the game
