...
const selectRow = (row) => {
  ...
  else if(!row.lastElementChild && stone) {
    dropStone(row.id)
  }
  else if(stone.getAttribute("id") < row.lastElementChild.getAttribute("id")) { 
    console.log(stone.getAttribute("id") + "<" +  row.lastElementChild.getAttribute("id"))
    dropStone(row.id) 
  }
  else {
