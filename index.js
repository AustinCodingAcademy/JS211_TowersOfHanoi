...
const selectRow = (row) => {
  ...
  console.log("Yay, we clicked an object", row);
...
const selectStone = (stone) => {
  let currentStone = stone.getAttribute("class")
...
const pickUpStone = (row,stone) => {
  let selectedRow = document.getElementById(row);
  let selectedBlock = document.getElementById([stone])
  // stone = selectedRow.removeChild(selectedRow.lastChild);
  console.log(stone[selectedBlock])
...
const dropStone = () => {
  ...
  stone = selectedRow.appendChild(selectedRow.firstChild);
};
